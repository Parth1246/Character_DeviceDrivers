...
/* Declare the probe and remove dunctions */
...
static int dt_probe(struct platform_device *pdev) {
	...
	printk("dt_probe - now I am in the probe functions!\n");

	/* Check for device proeprties */
	...
	if(ret) {
		printk("dt_probe - Error! Could not read 'lable'\n");
		return -1;
	}
 ...
 */
static int dt_remove(struct platform_device *pdev) {
	printk("dt_probe - Now I am in remove function\n");
	return 0;
}
...
static int __init ModuleInit(void) {
	...
	if(platform_driver_register(&my_driver)) {
		printk("dt_probe - Error! Could not load the driver\n");
		return -1;
	}
	return 0;
}
